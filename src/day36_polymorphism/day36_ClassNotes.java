...
2 types of type castings:
		...
		1. Primitive Type Castings: must happen between primitives
				...
				1. implicit primitive type casting: casting smaller primitive type to
				larger primitive type  (done implicitly)

								int a = 100;
								double b = a; // implcitly casted to double

				2. explicit primitive type casting: casting larger primitive type to
...
OOP Polymorphsim: parent type being referenced to the child object
				 ...
				 Reference type can be either parent class or parent interface, and object
				  must be created from child class

				 Reference Type decides what can be accessed/called
				 		(if we need to access the features that are in object type, but not
				 		in the reference type, then we need down casting)
